...
async function main() {
  console.log('======================================================================');
  console.log('🔬 GERKINK Phase P4 Firebase & Client Data-Flow Forensic Remeasurement');
  console.log(`Server: ${TEST_HOST} (Compiled Production Server with P4 Optimizations)`);
  ...
  try {
    for (const route of TARGET_ROUTES) {
      ...
      if (runs.length) {
        ...
        const fcpMedian = calculateMedian(fcps);
        const lcpMedian = calculateMedian(lcps);
        const tbtMedian = calculateMedian(tbts);
        ...
        const stats = {
          fcp: { median: fcpMedian, best: Math.min(...fcps), worst: Math.max(...fcps) },
          lcp: { median: lcpMedian, best: Math.min(...lcps), worst: Math.max(...lcps) },
          tbt: { median: tbtMedian, best: Math.min(...tbts), worst: Math.max(...tbts) },
        };

        console.log(`   📊 Summary [${route.name}]:`);
        console.log(`      FCP: ${stats.fcp.median}ms (best: ${stats.fcp.best}ms, worst: ${stats.fcp.worst}ms)`);
        console.log(`      LCP: ${stats.lcp.median}ms (best: ${stats.lcp.best}ms, worst: ${stats.lcp.worst}ms)`);
        console.log(`      TBT: ${stats.tbt.median}ms (best: ${stats.tbt.best}ms, worst: ${stats.tbt.worst}ms)`);
  ...
  const outputPath = 'scripts/p4-remeasurement-full.json';
