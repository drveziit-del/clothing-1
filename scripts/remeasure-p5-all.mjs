...
async function runAudit(url, chromePort) {
  ...
  try {
    ...
    // Attach response MIME types & sizes to images
...
async function main() {
  console.log('======================================================================');
  console.log('🔬 GERKINK P5 Asset & Media Pipeline Forensic Remeasurement');
  ...
  try {
    for (const route of TARGET_ROUTES) {
      console.log(`▶ Auditing [${route.name}] (${TEST_HOST}${route.path})...`);
      const runs = [];

      for (let r = 1; r <= 3; r++) {
        process.stdout.write(`   Run ${r}/3... `);
        try {
          const m = await runAudit(`${TEST_HOST}${route.path}`, chromePort);
          runs.push(m);
      ...
      if (runs.length) {
        ...
        results[route.slug] = {
          ...
          median: {
            ...
            lcpImageDetails: bestRun.lcpImageDetails,
            tbt: tbtMedian,
            ...
            imageAudits: bestRun.imageAudits,
          },
          runs,
        };
      }
  ...
  const outputPath = 'scripts/p5-remeasurement-full.json';
