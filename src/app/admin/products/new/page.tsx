...
function AddProductForm() {
  ...
  const [commitmentText, setCommitmentText] = useState('GERKINK stands for zero apologies and unapologetic self-expression. We produce all pieces in limited quantities to prevent overproduction and waste.');

  // UGC Videos Array
  const [ugcVideosList, setUgcVideosList] = useState<Array<{ name: string; stars: number; videoUrl: string }>>([]);
  const [newUgcName, setNewUgcName] = useState('');
  const [newUgcStars, setNewUgcStars] = useState(5);
  ...
  useEffect(() => {
    ...
    activeColors.forEach((c) => {
      activeSizes.forEach((s) => {
        ...
        const stableId = `variant_${c.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}_${s.toLowerCase().replace(/[^a-z0-9]/g, '_')}`;
        newList.push({
          ...
          images: (existing as any)?.images || undefined,
        });
      });
    });
  ...
  // Set default selected variant once variants load
  useEffect(() => {
            ...
            <div className={productStyles.layout}>
              ...
              <div className={productStyles.info}>
                ...
                <div style={{ borderTop: '1px solid #222', paddingTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  {/* Printify ID input inside Visual Mode */}
                  <div>
                    <label style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', display: 'block' }}>Printify Product ID (Required)</label>
                    <input
                      type="text"
                      value={printifyId}
                      onChange={(e) => setPrintifyId(e.target.value)}
                      placeholder="e.g. 64b8fbca928..."
                      style={{ background: 'transparent', border: '1px dashed #444', color: '#fff', outline: 'none', width: '100%', padding: '0.25rem', fontSize: '0.8rem' }}
                      required
                    />
                  </div>
                ...
                <div style={{ maxWidth: '600px', margin: '2rem auto 0 auto', background: '#111', padding: '1rem', border: '1px dotted #444', borderRadius: '4px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.5rem' }}>
                    <input
                      type="text"
                      value={newUgcName}
                      onChange={(e) => setNewUgcName(e.target.value)}
                    ...
                    <select
                      value={newUgcStars}
                      onChange={(e) => setNewUgcStars(Number(e.target.value))}
                      style={{ background: '#0a0a0a', border: '1px solid #333', color: '#fff', fontSize: '0.8rem', padding: '0.5rem' }}
                    >
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <input
                      type="url"
                      value={newUgcUrl}
                      onChange={(e) => setNewUgcUrl(e.target.value)}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (newUgcName.trim() && newUgcUrl.trim()) {
                          setUgcVideosList((prev) => [
                            ...prev,
                            {
                              name: newUgcName.trim(),
                              stars: newUgcStars,
                              videoUrl: newUgcUrl.trim(),
                            },
                          ]);
                          setNewUgcName('');
                          setNewUgcUrl('');
